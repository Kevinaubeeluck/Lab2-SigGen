// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Tracing implementation internals
#include "verilated_vcd_c.h"
#include "Vsingen__Syms.h"


VL_ATTR_COLD void Vsingen___024root__trace_init_sub__TOP__0(Vsingen___024root* vlSelf, VerilatedVcd* tracep) {
    if (false && vlSelf) {}  // Prevent unused
    Vsingen__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vsingen___024root__trace_init_sub__TOP__0\n"); );
    // Init
    const int c = vlSymsp->__Vm_baseCode;
    // Body
    tracep->declBit(c+1,"clk", false,-1);
    tracep->declBit(c+2,"rst", false,-1);
    tracep->declBit(c+3,"en", false,-1);
    tracep->declBus(c+4,"dout1", false,-1, 7,0);
    tracep->declBus(c+5,"dout2", false,-1, 7,0);
    tracep->declBus(c+6,"offset", false,-1, 7,0);
    tracep->pushNamePrefix("singen ");
    tracep->declBus(c+9,"WIDTH", false,-1, 31,0);
    tracep->declBit(c+1,"clk", false,-1);
    tracep->declBit(c+2,"rst", false,-1);
    tracep->declBit(c+3,"en", false,-1);
    tracep->declBus(c+4,"dout1", false,-1, 7,0);
    tracep->declBus(c+5,"dout2", false,-1, 7,0);
    tracep->declBus(c+6,"offset", false,-1, 7,0);
    tracep->declBus(c+7,"count", false,-1, 7,0);
    tracep->pushNamePrefix("Mycounter ");
    tracep->declBus(c+9,"WIDTH", false,-1, 31,0);
    tracep->declBit(c+1,"clk", false,-1);
    tracep->declBit(c+2,"rst", false,-1);
    tracep->declBit(c+3,"en", false,-1);
    tracep->declBus(c+7,"count", false,-1, 7,0);
    tracep->popNamePrefix(1);
    tracep->pushNamePrefix("Myrom ");
    tracep->declBus(c+9,"ADDRESS_WIDTH", false,-1, 31,0);
    tracep->declBus(c+9,"DATA_WIDTH", false,-1, 31,0);
    tracep->declBit(c+1,"clk", false,-1);
    tracep->declBus(c+7,"addr1", false,-1, 7,0);
    tracep->declBus(c+8,"addr2", false,-1, 7,0);
    tracep->declBus(c+4,"dout1", false,-1, 7,0);
    tracep->declBus(c+5,"dout2", false,-1, 7,0);
    tracep->popNamePrefix(2);
}

VL_ATTR_COLD void Vsingen___024root__trace_init_top(Vsingen___024root* vlSelf, VerilatedVcd* tracep) {
    if (false && vlSelf) {}  // Prevent unused
    Vsingen__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vsingen___024root__trace_init_top\n"); );
    // Body
    Vsingen___024root__trace_init_sub__TOP__0(vlSelf, tracep);
}

VL_ATTR_COLD void Vsingen___024root__trace_full_top_0(void* voidSelf, VerilatedVcd::Buffer* bufp);
void Vsingen___024root__trace_chg_top_0(void* voidSelf, VerilatedVcd::Buffer* bufp);
void Vsingen___024root__trace_cleanup(void* voidSelf, VerilatedVcd* /*unused*/);

VL_ATTR_COLD void Vsingen___024root__trace_register(Vsingen___024root* vlSelf, VerilatedVcd* tracep) {
    if (false && vlSelf) {}  // Prevent unused
    Vsingen__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vsingen___024root__trace_register\n"); );
    // Body
    tracep->addFullCb(&Vsingen___024root__trace_full_top_0, vlSelf);
    tracep->addChgCb(&Vsingen___024root__trace_chg_top_0, vlSelf);
    tracep->addCleanupCb(&Vsingen___024root__trace_cleanup, vlSelf);
}

VL_ATTR_COLD void Vsingen___024root__trace_full_sub_0(Vsingen___024root* vlSelf, VerilatedVcd::Buffer* bufp);

VL_ATTR_COLD void Vsingen___024root__trace_full_top_0(void* voidSelf, VerilatedVcd::Buffer* bufp) {
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vsingen___024root__trace_full_top_0\n"); );
    // Init
    Vsingen___024root* const __restrict vlSelf VL_ATTR_UNUSED = static_cast<Vsingen___024root*>(voidSelf);
    Vsingen__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    // Body
    Vsingen___024root__trace_full_sub_0((&vlSymsp->TOP), bufp);
}

VL_ATTR_COLD void Vsingen___024root__trace_full_sub_0(Vsingen___024root* vlSelf, VerilatedVcd::Buffer* bufp) {
    if (false && vlSelf) {}  // Prevent unused
    Vsingen__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vsingen___024root__trace_full_sub_0\n"); );
    // Init
    uint32_t* const oldp VL_ATTR_UNUSED = bufp->oldp(vlSymsp->__Vm_baseCode);
    // Body
    bufp->fullBit(oldp+1,(vlSelf->clk));
    bufp->fullBit(oldp+2,(vlSelf->rst));
    bufp->fullBit(oldp+3,(vlSelf->en));
    bufp->fullCData(oldp+4,(vlSelf->dout1),8);
    bufp->fullCData(oldp+5,(vlSelf->dout2),8);
    bufp->fullCData(oldp+6,(vlSelf->offset),8);
    bufp->fullCData(oldp+7,(vlSelf->singen__DOT__count),8);
    bufp->fullCData(oldp+8,((0xffU & ((IData)(vlSelf->offset) 
                                      + (IData)(vlSelf->singen__DOT__count)))),8);
    bufp->fullIData(oldp+9,(8U),32);
}
